...
void SendCommand(unsigned char cmd)
{
	...
	char temp[20];
						// data sheet page 3, 제공되는 프로토콜
	UART1_Putch(0x7E);	//	(start byte)
	UART1_Putch(0xFF);	// (Version)
	UART1_Putch(0x06);	//  (Len) data length 6으로 써라
	UART1_Putch(cmd);	// (commend)
	UART1_Putch(0x00);	// (Feed back)
	UART1_Putch(0x00);	// (parameter 1)

	if(cmd != CMD_TF){									//
		UART1_Putch(0x00);								//
		checksum = 0 - (0xFF + 0x06 + cmd);				//	parameter 2
	}													//
	else{												//
		UART1_Putch(0x02);								//
	...
	UART1_Putch(checksum & 0xFF);	// 그리고 나머지 checksom 하위비트 8개 마저 보냄. 이렇게 총 2byte를 보내는 게지.
								 //=====================================================================
	UART1_Putch(0xEF);			// 0xEF, end bit.	// 데이터 보낸것을 끝냄을 알림.
